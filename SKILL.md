...
除了全文翻译之外，你还需要在全文 Markdown 中嵌入一段“剥洋葱式文献导读”，帮助用户从动机、直觉、核心公式到剩余细节逐层理解论文。
...
- 全文翻译 Markdown，其中包含“剥洋葱式文献导读”版块
- 阅读笔记 Markdown
...
1. 一个以论文名称命名的统一输出目录
2. 结构保真的全文中文翻译
3. 图片、表格、公式的完整保留或可追溯降级保留
4. 嵌入在全文 Markdown 内的“剥洋葱式文献导读”
...
- 输出为纯中文，不做中英对照
- 笔记单独存为 .notes.md 文件
- “剥洋葱式文献导读”默认放在全文翻译文件末尾，位于参考文献之后
...
在全文翻译 Markdown 中追加一个独立章节，标题固定为：
...
- 这部分必须与全文翻译放在同一个 full.md 文件中，不得新建第三个 Markdown 文件
- 语气应偏讲解，不要写成论文式摘要复述
...
- 原论文文件是否已经移动到论文名称目录中
- full.md、notes.md、assets 是否都位于同一个论文名称目录中
- 所有图片链接是否存在且可相对访问
- 表格是否可读
- 公式是否保持原义和排版边界
- “剥洋葱式文献导读”是否已写入 full.md 而不是单独成文件
- 导读部分是否完整覆盖 4 个步骤
- 输出文件是否与原论文位于同一目录
...
- 用户在读完“剥洋葱式文献导读”后，能够先抓住直觉，再回到公式与全文
...
- 在全文翻译末尾追加“剥洋葱式文献导读”，但总共仍然只输出 full.md 和 notes.md 两个文件，原论文也要移动进同目录。
...
- “剥洋葱式文献导读”是放在全文末尾，还是放在全文开头导读后再进入正文

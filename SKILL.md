...
description: |
  将用户提供的论文完整翻译为中文 Markdown，尽量不遗漏正文内容，保留图题、表格与公式，
  并创建一个以论文名称命名的目录，将原论文与产物统一收纳在其中。
  Use when: 用户要求逐段全文翻译论文、保留公式与表格结构、输出 Markdown、并附带研究笔记。
license: MIT
metadata:
  author: workspace
  version: "0.4.0"
...
- 默认不导出任何图片文件，不创建 assets/ 目录
- 在 full.md 中保留图号、图题以及必要的上下文说明
- 如果某张图对理解方法非常关键，可在 notes.md 中用文字概述该图表达的机制
- 不要插入失效图片链接或占位图片路径
- 不要为了保图而额外生成截图文件
...
- 块级公式使用 $$...$$
- 行内公式使用 $...$
- 如果公式原本是图片或提取失败，先尝试手工转写为 LaTeX；若仍不可靠，则保留原始符号描述并标记该处为降级保留
...
- 原论文文件是否已经移动到论文名称目录中
- full.md、notes.md 和原论文是否都位于同一个论文名称目录中
- full.md 中是否不存在失效图片链接
- 表格是否可读
- 公式是否保持原义和排版边界
...
- 用户能在一个以论文名称命名的目录中同时找到原文、全文译文和笔记
- 图题、表格、公式在阅读位置上基本正确
...
- 把这篇 PDF 收进一个以论文名命名的目录里，原文件剪切进去，再生成全文中文 Markdown 和 notes，不要导出图片 assets。
- 处理这个 arXiv 论文链接，创建论文名称目录，并把下载的原论文、中文全文 md 和 notes 文件一起放进去。
- 对这篇论文做逐段翻译，不要省略 appendix，公式用 LaTeX，图题保留为文字说明，所有结果与原论文放在同一个论文名称目录里。
